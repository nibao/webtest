...
describe('登录首页', () => {
    ...
    describe('UI', () => {
        ...
        it('账号密码输入框', async () => {
            ...
            await getDiff(screenshotDir, '账号输入框.png')

            await passwordInputContainer.screenshot({
                path: `${screenshotDir}/密码输入框.png`
            })
            await getDiff(screenshotDir, '密码输入框.png')
        ...
        it('登录按钮', async () => {
            ...
            await getDiff(screenshotDir, '登录按钮.png')
        ...
        it('关于信息', async () => {
            ...
            await getDiff(screenshotDir, '关于信息.png')
            ...
            it('Office插件-点击下载', async () => {
                ...
                const promise = new Promise((resolve, reject) => {
                    page.once('request', request => {
                        if (request.url().indexOf('/download/AnyShare_for_Office_CN.exe') !== -1) {
                            resolve()
                        } else {
                            reject()
                        }
                ...
                promise.then(() => {
                    console.log('Office插件-点击下载，浏览器访问下载地址')
                }).catch(() => {
                    throw new Error('Office插件-点击下载，浏览器未访问下载地址')
                })
            })
