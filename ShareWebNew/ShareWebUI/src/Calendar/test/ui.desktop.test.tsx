...
import Calendar from '../ui.desktop';
    ...
    describe('Calendar@desktop', () => {
        ...
        describe('#onSelect', () => {
            it('选择日期', () => {
                ...
                wrapper.find('tbody > tr').at(2).find('td').at(4).find('LinkChip').simulate('click');
            ...
            it('设置选择范围时非选择范围内的日期无法点击', () => {
                ...
                wrapper.find('tbody LinkChip').forEach((node, index) => {
                    index < 9 && node.simulate('click')
                })
                expect(spy.called).to.be.false

                /* 点击10-12日中任意一天触发选择事件 */
                wrapper.find('tbody LinkChip').at(9).simulate('click');
                expect(spy.calledOnce).to.be.true
                wrapper.find('tbody LinkChip').at(10).simulate('click');
                expect(spy.calledTwice).to.be.true
                wrapper.find('tbody LinkChip').at(11).simulate('click');
                expect(spy.calledThrice).to.be.true

                /* 点击13-28日任意一天，不触发选择事件 */
                wrapper.find('tbody LinkChip').forEach((node, index) => {
                    index > 13 && node.simulate('click')
                })
                expect(spy.calledThrice).to.be.true
