 ...
 * @param Message // 提示信息 
 */
export default function Failed({ failedSyncs = [],
    failedNum = 0,
    clear = noop,
    openDirByRelPath = noop,
    openDirByAbsPath = noop,
    deleteByLogId = noop,
    showTaskIcon = noop }: Components.Sync.Failed.Props) {
    return (
        <div className={styles['head']}>
            <div className={styles['left']}>
                {
                    __('共有 ${total} 个任务同步失败', { total: failedNum === 0 ? '0' : failedNum })
                }
            </div>
            <div className={styles['right']} >
                <Button disabled={failedNum === 0} className={styles['button']} onClick={clear.bind(this)}>
                    {__('清空记录')}
                </Button>
            </div>
